...
public class RecipientsTable {
  ...
  public Recipient get(String e164, UUID queryUUID) throws SQLException, IOException {
    ...
    if (results.size() > 0) {
      ...
      UUID storedUUID = result.getAddress() != null ? result.getAddress().getUuid() : null;
      String storedE164 = result.getAddress() != null ? result.getAddress().getNumber().orNull() : null;
      ...
      // query did not include an e164, but the database provided it
      if (e164 == null && storedE164 != null) {
        e164 = storedE164;
      }
      ...
      if (queryUUID != null && storedUUID == null) {
        update(UUID, queryUUID.toString(), result.getId());
      }

      // query did not include UUID, but the database provided it
      if (queryUUID != null && storedUUID != null) {
        queryUUID = storedUUID;
      }
    }

    if (queryUUID == null) {
      queryUUID = getRegisteredUser(e164);
      if (rowid > 0) {
